...
이러한 특성 덕분에 구조체(struct)나 열거형(enum)과 같은 값 타입은 (모든 내부 상태가 `Sendable`을 준수하는 한) 기본적으로 `Sendable`로 간주됩니다. 구조체의 내부 모든 상태가 `Sendable`을 준수한다면, 해당 구조체는 `Sendable`을 암시적으로 채택합니다.
...
```
하지만 구조체 내부에 하나라도 `Sendable`을 준수하지 않는 프로퍼티가 있다면, 해당 구조체 전체는 `Sendable`이 될 수 없습니다. 즉, 값 타입이라 하더라도 내부에 비동기 환경에서 안전하지 않은 참조 타입이 포함되어 있다면, 해당 타입은 서로 다른 스레드 간에 안전하게 공유될 수 없습니다. 
...
> 💡 **Note:** 구조체의 접근 제어자가 `public`인 경우, 명시적으로 `Sendable` 프로토콜을 채택해야 합니다.
...
Swift에서는 함수(특히 클로저)도 `Sendable`하게 만들 수 있습니다. 하지만 클로저는 일반 타입처럼 `Sendable` 프로토콜을 직접 채택할 수 없기 때문에, 대신 `@Sendable`이라는 특별한 속성(Attribute)이 도입되었습니다.
...
```swift
@MainActor var mutable = 30
@MainActor let immutable = 10

let sendableClosure = { @Sendable () -> Void in
//    print("➡️ @Sendable 클로저는 가변 상태를 캡처(Capture)할 수 없습니다. \(mutable)")
    print("➡️ @Sendable 클로저는 `Sendable`한 불변 상태만 캡처할 수 있습니다. \(immutable)")
}
...
>//    print(counter) // 🔴 메인 액터에 격리된 `counter` 프로퍼티는 `@Sendable` 클로저에서 접근할 수 없음
>}
>
>var asyncClosure: (@Sendable () async -> Void) = {
>    print(await counter)
>}
>```
>
