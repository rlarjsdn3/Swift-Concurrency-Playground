...
첫 번째 경우부터 살펴보겠습니다. 메인 액터에서 일반 액터로 액터 홉핑이 발생하는 경우, 이때는 단순한 실행 컨텍스트가 전환됩니다. 그 이유는, 특히 메인 액터와 일반 액터 간의 전환에서는 메인 스레드와 협력형 워커 스레드 풀이 서로 분리된 실행 환경을 사용하기 때문에, 실행 스레드가 바뀌는 경우가 흔합니다.
